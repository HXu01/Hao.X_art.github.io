...
if (keyIsPressed) {
    if (key === ']') {
      ...
      clear_print();
    } else if (key === 'x' || key === 'X') {  // Added clear canvas function
      background(220);  // Clear to match initial background color
    }
  }
...
function HX_Pen_1() {
  ...
  for (let i = 0; i < trail.length - 1; i++) {
      fill(trail[i].color);
      // Draw a line between points to fill gaps
      let next = trail[i + 1];
      let steps = 10; // Increased number of steps for smoother line
      for (let j = 0; j <= steps; j++) {
          let x = lerp(trail[i].x, next.x, j/steps);
          let y = lerp(trail[i].y, next.y, j/steps);
